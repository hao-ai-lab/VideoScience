...
REQUIRED_COLUMNS = [
    ...
    "Prompts",              # -> --description
    "Source",               # -> (kept for schema; not used for path)
...
def uid_to_folder(uid: str, prefix: str = "vid_", pad: int = 3) -> str:
    ...
    # also accept strings like 'vid_50' / 'vid_050'
...
def infer_ref_path_for_vid(ref_dir: Optional[Path], vid_num: int) -> Optional[str]:
    """Try both non-padded and padded reference names."""
    if not ref_dir:
        return None
...
def main():
    ...
    ap.add_argument("--out-dir", default="judge/results/evaluation_videos", help="Base results dir.")
    ap.add_argument("--max-runs", type=int, default=0, help="Cap total runs (0=no cap).")
    ap.add_argument("--script", default="judge/vlm_as_a_judge.py", help="Judge script path.")
    ap.add_argument("--reference-dir", default="", help="Dir containing vid_{id}_ref.mp4 files.")
    ap.add_argument("--runner-name", default="run_all.sh", help="Output bash filename (default: run_all.sh).")

    # filters
    ap.add_argument("--mode", choices=["ready", "all"], default="all",
    ...
    # source scanning
    ap.add_argument("--source-dir", required=True, help="Root directory containing model subfolders with run videos.")
    ap.add_argument("eval_target", help="Subfolder under --source-dir to scan (e.g., sora2).")
    ...
    # ---- apply filters (exact column names) ----
    if args.mode == "ready":
    ...
    # Build a mapping from normalized vid integer -> row info (phenomenon, description, uid_folder)
    selected_by_vid_int: Dict[int, Dict[str, str]] = {}
    for r in rows:
        uid_raw = (r.get("Unique ID") or "").strip()
        vid_num = vid_int_from_any(uid_raw)
        if vid_num is None:
            # fallback: try from Example Title if it contains something like vid_### (rare)
            vid_num = vid_int_from_any(r.get("Example Title", ""))
        if vid_num is None:
            continue
        selected_by_vid_int[vid_num] = r  # last wins if duplicates

    # Nothing selected after filters? Emit empty runner.
    base_out = Path(args.out_dir).resolve()
    base_out.mkdir(parents=True, exist_ok=True)
    model_slug = slugify(args.model)
    runner_path = base_out / args.runner_name

    # Prepare reference dir
    ref_dir = Path(args.reference_dir).resolve() if args.reference_dir else None

    # Prepare scan root: source_dir/<eval_target>
    ...
    run_lines.append("")
    run_lines.append(f'MAX_RUNS="${{MAX_RUNS:-{args.max_runs}}}"' if args.max_runs > 0 else 'MAX_RUNS="${MAX_RUNS:-0}"')
    ...
    for p in sorted(source_root.rglob("*.mp4")):
        ...
        # Keep only those vids that passed CSV filters (author/mode)
        row = selected_by_vid_int.get(vid_num)
        if not row:
            continue

        # Pull description + expected phenomenon from the CSV row
        phenomenon = (row.get("Expected phenomenon") or "").strip()
        description = (row.get("Prompts") or "").strip()
        if not (phenomenon and description):
            continue

        # Output folder name from CSV UID with padding
        uid_folder = uid_to_folder((row.get("Unique ID") or "").strip(), prefix="vid_", pad=3)
        out_dir = base_out / model_slug / uid_folder / f"run_{run_idx}"

        # Infer reference path for this vid (try non-padded and padded)
        ref_path = infer_ref_path_for_vid(ref_dir, vid_num) if ref_dir else None
        ref_arg = f" --ref_video {shlex.quote(ref_path)}" if ref_path else ""
    ...
    summary = {
        ...
        "mode": args.mode,
        "author_filter": [a for a in (args.author.split(",") if args.author else []) if a.strip()],
    }
    print(json.dumps(summary, indent=2))
    print(f"\nUsage:\n  {runner_path}           # uses built-in cap or MAX_RUNS env\n  MAX_RUNS=3 {runner_path}")
