...
def create_checklist_prompt(prompt_text, reference_source ="", additional_reference_from_multimedia=""):
...
Reference ground truth phoenonemnon: {reference_source} 

Additional information about the multimedia: {additional_reference_from_multimedia}

Use reference sources to create the checklist (and the reference video if available)
...
3. SPATIO-TEMPORAL CONHERENCE: Are spatial relationships and temporal sequences physically consistent?

4. IMMUTABILITY: Do object properties remain physically consistent?

5. CORRECT DYNAMISM: Do object interactions follow other physical laws not examined in the expected phenomenon?

Guidelines for checklist creation:
- only target things which are visually observable in the video
- the statements in checklist needs to be assertive statements that can be answered with YES or NO instead of questions
"""
...
def process_csv_and_generate_checklists(csv_path, output_folder, author, model="models/gemini-2.5-flash"):
    ...
    with open(csv_path, 'r', encoding='utf-8') as csvfile:
        ...
        for idx, row in enumerate(reader, start=1):
            ...
            reference_phenomenon = row.get('Expected phenomenon', '').strip()
            ...
            try:
                ...
                if "youtube.com" in source_link or "youtu.be" in source_link:
                    ...
                    full_prompt_with_ref = create_checklist_prompt(prompt_text, reference_source=reference_phenomenon, additional_reference_from_multimedia=reference_section)
